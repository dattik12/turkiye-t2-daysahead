...
def _hub_pull(lat: float, lon: float, s: str, e: str, url: str,
...
def history_wind(s: str, e: str, matrix=None, model: str | None = None,
                 suffix: str = "") -> pd.DataFrame:
    """Saatlik hub ruzgari (m/s), long format: dt/city/w100/w10/wdir.
    model+suffix ile ikinci NWP (orn. GFS) cekilir; kolonlar w100{suffix} olur."""
    matrix = load_matrix() if matrix is None else matrix
    out = []
    for _, r in matrix.iterrows():
        h = _hub_pull(r["lat"], r["lon"], s, e, C.OM_HISTO_URL, model=model).sort_index()
        h = h[~h.index.duplicated(keep="last")]
        h["city"] = r["province"]
        h = h.rename(columns={f"wind_speed_100m": f"w100{suffix}",
                              f"wind_speed_10m": f"w10{suffix}",
                              f"wind_direction_100m": f"wdir{suffix}"})
        h[[f"w100{suffix}", f"w10{suffix}"]] = h[[f"w100{suffix}", f"w10{suffix}"]] * KMH_TO_MS
        h["dt"] = h.index
...
def forecast_wind(s: str, e: str, matrix=None, model: str | None = None,
                  suffix: str = "") -> pd.DataFrame:
    """Hedef gunler icin hub IFS tahmini, long format (history_wind ile ayni sema)."""
    matrix = load_matrix() if matrix is None else matrix
    out = []
    for _, r in matrix.iterrows():
        h = _hub_pull(r["lat"], r["lon"], s, e, C.OM_FC_URL, model=model).sort_index()
        h = h[~h.index.duplicated(keep="last")]
        h["city"] = r["province"]
        h = h.rename(columns={f"wind_speed_100m": f"w100{suffix}",
                              f"wind_speed_10m": f"w10{suffix}",
